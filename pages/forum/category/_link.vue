    ...
    <v-card flat tile v-else color="card" >
        ...
        <v-card-text v-if="Topics.length < 1">
            <v-alert tile type="info" dense prominent elevation="0" class="my-0" color="cardmini">
                Hiện tại chưa có bài viết nào trong khu vực
            </v-alert>
        </v-card-text>
...
export default {
    ...
    async fetch() {
        try {
            let Get = await this.$axios.$get(`/forum/category/${this.$route.params.link}/0`);
            if(Get.err) throw Get.status;

            this.Topics = Get.topics;
            this.Category = Get.category;

            this.$store.commit('forum/SetCategory', this.Category)
        } catch(e) {
    ...
    beforeRouteLeave (to, from, next) {
        this.$store.commit('forum/SetCategory', null);
        next()
    },
    data() {
        return {
